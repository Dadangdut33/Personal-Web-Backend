...
const badRequestName = ["ValidationError", "CastError", "BSONError"];
const badRequestMessage = ["Argument passed", "Expected a positive"];
export const getDataStatus = (res: Response, err: Error) => {
  const statusCode =
    res.statusCode !== 200
      ? res.statusCode
      : badRequestName.includes(err.name) || badRequestMessage.some((subString) => err.message.includes(subString))
        ? 400
        : 500;
  const dataErr = {
