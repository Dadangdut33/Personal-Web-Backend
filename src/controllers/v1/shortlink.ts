...
import { IShortLinkModel, shortLinkModel } from "../../models/shortlink";
import { ___issue___, colShortLink, colUser, unsetAuthorFields } from "../../utils";
...
export const clickCountsOnly = async (_req: Request, res: Response) => {
  // sum all clickCounts
  const clickCounts = await shortLinkModel
    .aggregate([{ $match: {} }, { $group: { _id: null, clickCount: { $sum: "$clickCount" } } }])
    .exec();
  return res.status(200).json({
...
export const getShortlinkStats = async (_req: Request, res: Response) => {
  const stats = await shortLinkModel.db.db.command({ collStats: colShortLink });
  return res.status(200).json({
    data: stats,
...
export const createShortLink = async (req: Request, res: Response) => {
  ...
  const shortLink = await shortLinkModel.create({ author: new Types.ObjectId(req.session.userId), url, shorten });
  return res.status(!!shortLink ? 201 : 500).json({
    data: shortLink,
    message: !!shortLink
      ? "ShortLink created successfully"
      : `Unable to create shortlink. If you think that this is a bug, please submit an issue at ${___issue___}`,
    success: !!shortLink,
  });
};
...
export const updateShortLink = async (req: Request, res: Response) => {
  const { _id } = req.params;
  const { url } = req.body;
  const shortLink = await shortLinkModel.findByIdAndUpdate(
    { _id },
    { url, editedBy: new Types.ObjectId(req.session.userId) },
...
export const deleteShortLink = async (req: Request, res: Response) => {
  const { _id } = req.params;
  const shortLink = await shortLinkModel.findByIdAndDelete({ _id });
  return res.status(!!shortLink ? 200 : 422).json({
    data: shortLink,
