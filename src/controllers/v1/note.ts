...
import { INoteModel, noteModel } from "../../models/note";
import { ___issue___, colUser, unsetAuthorFields } from "../../utils";
...
export const createNote = async (req: Request, res: Response) => {
  ...
  const note = await noteModel.create({ ...req.body, position: position, author: new Types.ObjectId(req.session.userId) });
  return res.status(!!note ? 201 : 500).json({
    data: note,
    message: !!note
      ? "Note created successfully"
      : `Unable to create note. If you think that this is a bug, please submit an issue at ${___issue___}`,
    success: !!note,
  });
};
