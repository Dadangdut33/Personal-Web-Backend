...
export const getOneUser_public = async (req: Request, res: Response) => {
	...
	const user = (await userModel.aggregate([{ $match: { username: username } }, { $unset: ["hash", "salt", "createdAt", "updatedAt"] }]).exec())[0] as IUserModel;
...
export const getOneUser_protected = async (req: Request, res: Response) => {
	...
	const user = (await userModel.aggregate([{ $match: { _id: Types.ObjectId(_id) } }, { $unset: ["hash", "salt"] }]).exec())[0] as IUserModel;
